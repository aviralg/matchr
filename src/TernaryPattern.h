#ifndef MATCHR_TERNARY_PATTERN_H
#define MATCHR_TERNARY_PATTERN_H

#include "Pattern.h"
#include <vector>

class TernaryPattern: public Pattern {
  public:
    TernaryPattern(SEXP r_expression,
                   Pattern* first_sub_pattern,
                   Pattern* second_sub_pattern,
                   Pattern* third_sub_pattern)
        : Pattern(r_expression)
        , first_sub_pattern_(first_sub_pattern)
        , second_sub_pattern_(second_sub_pattern)
        , third_sub_pattern_(third_sub_pattern) {
    }

    virtual ~TernaryPattern() {
        delete first_sub_pattern_;
        delete second_sub_pattern_;
        delete third_sub_pattern_;
    }
    ...
    const Pattern* get_third_sub_pattern() const {
        return third_sub_pattern_;
    }

    Pattern* get_third_sub_pattern() {
        return third_sub_pattern_;
    }
    ...
    IdentifierNames get_identifier_names() const override final {
        ...
        names.merge(get_third_sub_pattern()->get_identifier_names());
  ...
  private:
    ...
    Pattern* third_sub_pattern_;
};

#endif /* MATCHR_TERNARY_PATTERN_H */
